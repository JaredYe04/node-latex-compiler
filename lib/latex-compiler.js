...
function getTempDir () {
  ...
  if (fs.existsSync(tempDir)) {
    ...
    if (!stats.isDirectory()) {
      throw new Error(`ENOTDIR: "${tempDir}" exists but is not a directory. Please remove this file and try again.`)
    }
  } else {
...
class LatexCompiler {
  ...
  ensureDirectoryExists (dirPath) {
    if (fs.existsSync(dirPath)) {
      ...
      if (!stats.isDirectory()) {
        throw new Error(`ENOTDIR: "${dirPath}" exists but is not a directory. Please remove this file and try again.`)
      }
    } else {
      // Before creating directory recursively, check all parent directories
      // to ensure none of them are files (which would cause ENOTDIR)
      ...
      if (parentPath !== dirPath && parentPath !== '.' && parentPath !== '..') {
        ...
        if (fs.existsSync(parentPath)) {
          ...
          if (!parentStats.isDirectory()) {
            throw new Error(`ENOTDIR: "${parentPath}" exists but is not a directory. Cannot create "${dirPath}". Please remove this file and try again.`)
          }
        }
